...
| information resource / knowledge base reference | 情報資料 / 知識ベースへの参照 | media_file misuse の参照先の意味。CLUSTER.information_resource / CLUSTER.knowledge_base_reference は英字のまま（校正 2026-09-19 で確定。他候補：情報資源 / ナレッジベース参照） |
| healthcare process / health record | 医療プロセス / 健康記録 | media_file.v1（2026-09-19）。health record は person.v1 misuse と同じ |
| RM attributes / Reference Model attributes | 参照モデルの属性 | media_file.v1。organisation.v1 misuse（校正済み）に合わせる |
| Multimedia data type | Multimedia データ型 | DV_MULTIMEDIA を指す。データ型名は英字のまま（person.v1 の「DV_IDENTIFIER データ型」と同じ扱い） |
...
| temple / forehead (temperature site) | 側頭部 / 前額部 | body_temperature.v2 at0060 / at0061（校正 2026-09-22 で確定。こめかみ / 額 は不採用） |
| confounding factors | 交絡因子 | body_temperature.v2 at0066。CKM 既存の body_weight.v2 at0025 の ja 訳も「交絡因子」（前例あり） |
| location of measurement / structured measurement location | 測定部位 / 構造化された測定部位 | body_temperature.v2 at0021 / at0064。校正 2026-09-22 で既存訳の「計測」を「測定」に統一（measure＝測定） |
| device (measurement) | 機器 | body_temperature.v2 at0059（既存訳）。media_file の Source device＝作成機器 とは別 |
| inguinal skin crease | 鼠径溝 | body_temperature.v2 at0055。校正 2026-09-22 で既存訳「鼠径ひだ状皮膚」から変更 |
...
| 5th intercostal space | 第 5 肋間 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| cubital fossa | 肘窩 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| costal margin | 肋骨弓 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| McBurney's point | マックバーニー点 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| upper right quadrant | 右上腹部 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| interphalangeal joint of the great toe | 母趾の趾節間関節 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| vermilion border | 赤唇縁 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| digit | 指（手指・足趾） | anatomical_location.v1（1.5.1）。指定訳を採用 |
| laceration | 裂傷 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| tenderness | 圧痛 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| lower limb | 下肢 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| umbilicus | 臍 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| axilla | 腋窩 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| axillary skinfold | 腋窩ひだ | anatomical_location.v1（1.5.1）。指定訳を採用 |
| clavicle | 鎖骨 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| pupil | 瞳孔 | anatomical_location.v1（1.5.1）。指定訳を採用 |
| scapula | 肩甲骨 | anatomical_location.v1（1.5.1）。指定訳を採用 |
...
| social summary | 社会歴要約 | 要確認: 社会歴要約（他候補: 社会的状況の要約）Issue #44 の候補を採用し、臨床要約と名称を揃える。 |
| social history | 社会歴 | 要確認: 社会歴（他候補: 生活歴）社会的状況や経験を広く含む語として指定候補を採用する。 |
| social circumstances / social experiences | 社会的状況 / 社会的経験 | 要確認: 社会的状況／社会的経験（他候補: 社会生活の状況／経験）人間関係や居住、就労などを広く含む表現とする。 |
| lifestyle | 生活習慣 | 要確認: 生活習慣（他候補: ライフスタイル）飲酒・喫煙・食事・身体活動を指す文脈で指定候補を採用する。 |
| living arrangements | 居住状況 | 要確認: 居住状況（他候補: 生活環境、同居状況）住まいと同居者の状況を含む表現として指定候補を採用する。 |
| occupation / employment | 職業 / 就労 | 要確認: 職業／就労（他候補: 職種／雇用）職業の種類と働くことを区別する。social_summary.v1 keywords / use。 |
| social supports | 社会的支援 | 要確認: 社会的支援（他候補: ソーシャルサポート）制度による支援だけでなく周囲の人による支援も含む。 |
| relationships with others | 他者との関係 | 要確認: 他者との関係（他候補: 対人関係）関係の種類を限定しない表現とする。 |
| social / family / education / environment / housing / finances / religion | 社会 / 家族 / 教育 / 環境 / 住居 / 経済状況 / 宗教 | 要確認: 経済状況（他候補: 家計）finances は収入・資産などを含む広い表現とする。他は social_summary.v1 の一般語。 |
| alcohol, tobacco and other substance use | 飲酒、タバコの使用、その他の物質使用 | 要確認: 飲酒、タバコの使用、その他の物質使用（他候補: アルコール、タバコ、その他の物質の使用）飲酒は自然な表現とし、タバコは喫煙以外の使用も含める。 |
| diet and nutrition / physical activity | 食事と栄養 / 身体活動 | 要確認: 食事と栄養／身体活動（他候補: 食生活と栄養／運動）diet を減量食、physical activity を運動に限定しない。 |
...
| openEHR-EHR-EVALUATION.social_summary.v1 (1.1.1) | 社会歴要約 | Issue #44。要確認: 社会歴要約（他候補: 社会的状況の要約）指定候補を採用し、臨床要約と名称を揃える。 |
