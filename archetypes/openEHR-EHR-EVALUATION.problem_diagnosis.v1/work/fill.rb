...
T = {
  ...
  ['term', 'at0012', 'comment'] => "可能な限り、解剖学的部位の名称をTerminologyを用いてコード化することが推奨される。\n\n事前に調整された（プリコーディネートされた）解剖学的部位を記録する際には、このデータエレメントを使用する。解剖学的部位の記録要件が実行時にアプリケーションによって決定される場合や、相対的な位置関係などより複雑なモデリングが必要な場合は、このarchetype内の「Structured anatomical location（構造化された解剖学的部位）」スロットにある CLUSTER.anatomical_location または CLUSTER.relative_location を使用すること。このデータエレメントのCardinalityは無制限とされており、複数の部位に発疹があるものの、それ以外の属性がすべて同一であるといった臨床シナリオにも対応可能である。もし解剖学的部位が、事前調整されたコードによって「プロブレム・診断名」に含まれている場合、このデータエレメントは冗長となる。",
