...
T = {
  ...
  ['description', '-', 'misuse'] => "特定の構造化された健康情報を記録するために用いてはならない。例えば、プロブレム、診断、検査結果についての詳細な情報は、それぞれに対応する EVALUATION.problem、EVALUATION.problem-diagnosis、および OBSERVATION の臨床検査や放射線検査の結果の各archetypeを用いて記録すべきである。臨床要約は、要約の完全性のために重要と判断される場合には、これらの構造化された詳細の中から重要で選択された数値結果を伝えてもよいが、それらの主たる記録場所ではない。\n「臨床要約（Clinical Synopsis）」という語は、退院時サマリーや報告書のような複雑で包括的な文書を指すこともある。openEHR では、これらの文書は制約されたarchetypeの集合、すなわち複数の独立したarchetypeからなる退院時サマリーのテンプレートや報告書のテンプレートとして表現されるべきであり、本臨床要約archetypeはその一つになりうる。",
