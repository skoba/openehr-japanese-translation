...
T = {
  ...
  ['description', '-', 'misuse'] => "ISO 規格に記述されているような完全に構造化された住所の記録や、公式のデモグラフィック登録簿やインデックスの維持・管理の一環としての記録に用いてはならない。これらの目的には、住所検索サービス、正式な患者マスターインデックス（MPI）や「医療従事者名簿（Health Provider Index）」、または openEHR デモグラフィック情報モデルに基づくarchetypeを用いること。\n\n電子的な連絡先に用いるデジタルアドレスの表現に用いてはならない。デジタルアドレスの記録には、CLUSTER.electronic_communication を用いること。\n\n住居やホームレス状態に関する詳細情報の記録に用いてはならない。これらの記録には、EVALUATION.housing_summary を用いること。",
