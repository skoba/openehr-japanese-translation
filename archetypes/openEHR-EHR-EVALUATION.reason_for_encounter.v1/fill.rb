...
T = {
  ...
  ['description', '-', 'keywords'] => '受診理由 | 来院理由 | 主訴 | 受診時の問題 | 受診区分 | 理由 | 受診',
  ...
  ['term', 'at0004', 'text'] => '受診時の問題',
  ['term', 'at0004', 'description'] =>
    'ケア対象者が医療を求める動機となっている臨床的または社会的な問題の特定。',
  ['term', 'at0004', 'comment'] =>
    "可能であれば「受診時の問題」は用語集によりコード化することが望ましい。医療を求める臨床的または社会的な理由には、健康上の問題、症状、身体的徴候が含まれうる。例：健康上の問題 - 禁煙の希望、家庭内暴力；症状 - 腹痛、息切れ；身体的徴候 - 意識状態の変化。テンプレートでは「主訴」を「受診時の問題」の同義語として用いてよい。\n "
}.freeze

NOTES = {
  ['term', 'at0000', 'text'] => '「受診理由」で確定（encounter＝診察の規則の例外として概念名のみ慣用を優先）',
  ['term', 'at0002', 'text'] => 'contact＝受診 の規則により「受診区分」',
  ['term', 'at0004', 'text'] => '要確認: 「受診時の問題」を採用し、comment で「主訴」を同義語として明記（de 訳は Hauptbeschwerde=主訴を採用）。社会的問題や徴候も含む原義を優先',
