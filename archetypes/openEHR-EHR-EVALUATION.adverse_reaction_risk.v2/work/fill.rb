...
T = {
  ...
  ['term', 'at0002', 'comment'] => "「物質」には、個別の物質と物質の分類のどちらも記録できる。医薬品のように、複数の単純な物質からなるものもある。可能であれば、意思決定支援を作動させることができる用語集により「物質」をコード化することを強く推奨する。例：Snomed CT、DM+D、RxNorm、NDFRT、ATC、New Zealand Universal List of Medicines、Australian Medicines Terminology。自由記述は、適切な用語集を利用できない場合にのみ用いる。",
